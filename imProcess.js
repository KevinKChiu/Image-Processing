...
// ===YOUR CODE STARTS HERE===
	let skernel = [[0, -sharpness, 0], [-sharpness, 1 + 4 * sharpness, -sharpness], [0, -sharpness, 0]];
...
// ===YOUR CODE STARTS HERE===
	...
	for (let i = 0; i < num; i++) {
		...
		let denom_g = 0;
		let denom_b = 0;
		let pix = mimages[i].pixels;
		for (let y = 0; y < h; y++) {
			for (let x = 0; x < w; x++) {
				let idx = (y * w + x) * 4;
				denom_r += pix[idx + 0] * pix[idx + 0];
				denom_g += pix[idx + 1] * pix[idx + 1];
				denom_b += pix[idx + 2] * pix[idx + 2];
			}
		}
		denoms.push([denom_r, denom_g, denom_b]);
	}
...
// ===YOUR CODE STARTS HERE===
			for (let k = 0; k < num; k++) {
				let d_r = d_g = d_b = 0;
				let numerd_r = numerd_g = numerd_b = 0;
				let scale_r = scale_g = scale_b = 0;
				let sum_r = sum_g = sum_b = 0;
				let mpix = mimages[k].pixels;
				for (let j = 0; j < h; j++) {
					for (let i = 0; i < w; i++) {
						let midx = (j * w + i) * 4;
						let bidx = ((y + j) * input.width + (x + i)) * 4;
						sum_r += ip[bidx + 0] * mpix[midx + 0];
						sum_g += ip[bidx + 1] * mpix[midx + 1];
						sum_b += ip[bidx + 2] * mpix[midx + 2];
					}
				}
				numerd_r = -(sum_r * sum_r);
				numerd_g = -(sum_g * sum_g);
				numerd_b = -(sum_b * sum_b);

				d_r = numerd_r / denoms[k][0];
				d_g = numerd_g / denoms[k][1];
				d_b = numerd_b / denoms[k][2];

				scale_r = sum_r / denoms[k][0];
				scale_g = sum_g / denoms[k][1];
				scale_b = sum_b / denoms[k][2];

				let temp_d = d_r + d_g + d_b;
				...
				if (k === 0) {
					match_d = temp_d;
					match_index = k;
					match_scales = [scale_r, scale_g, scale_b];
				} else {
					if (temp_d < match_d) {
						match_d = temp_d;
						match_index = k;
			...
			let match_pix = mimages[match_index].pixels;
			for (let j = 0; j < h; j++) {
				for (let i = 0; i < w; i++) {
					let midx = (j * w + i) * 4;
					let bidx = ((y + j) * input.width + (x + i)) * 4;
					op[bidx + 0] = match_scales[0] * match_pix[midx + 0];
					op[bidx + 1] = match_scales[1] * match_pix[midx + 1];
					op[bidx + 2] = match_scales[2] * match_pix[midx + 2];
				}
			}
